...
public final class VMPageableQuery<RequestContext, Pageable: PageableAtsaniKeyProtocol, Response: Codable & Collection>: ObservableObject, VMQueryProtocol, VMQueryInvalidateListener {
  ...
  @Published public private(set) var currentPage: Pageable
  ...
  public func requery(forRequestContext requestContext: RequestContext) {
    ...
    self.currentPage = self.currentPage.first
  ...
  public func nextPage() {
    guard let requestContext = self.lastRequestContext else {
      return
    }
